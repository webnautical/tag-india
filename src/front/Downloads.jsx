...
import { useGetDownloadsQuery } from "../api/TagIndiaAPI"; // adjust path to your RTK api slice
...
export const Downloads = () => {
  const { data, isLoading, isError } = useGetDownloadsQuery();

  const downloads = (data?.data ?? []).filter((item) => item.is_active);
                ...
                <tbody>
                  {isLoading ? (
                    ...
                    downloads.map((item, i) => (
                      <tr key={item.id} className="hover:bg-[#f1e6f94d] transition-colors">
